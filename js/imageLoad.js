...
function setLogoCompanyLink(logoArray) {
	logoArray.map(logo => {
		...
		if (logoUrl === "" || !isValidURL(logoUrl)) {
			logo.url = `https://www.google.com/search?q=${encodeURIComponent(
				logo.title
			)}`;
...
function createLogos(logoArray) {
	...
	sortObjectArray(logoArray, "title").forEach(logo => {
		...
		if ("0123456789".includes(logo.title.charAt(0)))
		...
		const html =
			groupanchor +
			`<div class='logo'> <div class='logo__holder'> <div class='logo__image'> <img loading="lazy" src='logos/${
				logo.filename
			}/${logo.filename}.svg' alt='${
				logo.title
			}logo'> </div><div class='logo__download'> <div class='logo__download__overlay'> <a href='logos/${
				logo.filename
			}/${logo.filename}.svg' download='${
				logo.title
			} Logo.svg'> <span class='logo__download__overlay--svg'> Download SVG </span> </a> <a href='logos/${
				logo.filename
				...
				logo.title
			}</a></p><p class='logo__text--secondary'>${(logo.category + "").replace(
				",",
				" / "
			)}</p></div></div>`;
